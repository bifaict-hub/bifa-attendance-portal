  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        // Filter out empty rows — reg no in col A, name in col B, group in col C or D
        const data = rows
          .filter(r => r[0] && r[1])
          .map(r => ({
            reg_no: String(r[0]).trim(),
            name: String(r[1]).trim(),
            groupName: String(r[2] || r[3] || "").trim()  // col C first, fallback to col D
          }))
